...
allstandata_input <- lapply(allstandata_species, function(x) {
	...
	with(x, list(N=nrow(x), Nspp=8, Nyear=20, Nplot=max(plotfactor), Ntree=max(idfactor), targetsp=editedspnum[1], dbh=dbh/10, growthrate=growthrate, year=year, plot=plotfactor, tree=idfactor, gdd=gdd/1000, precip=precip/1000, areaxdist=as.matrix(x[,6:13]/1000), trait=as.matrix(traits_stan[,1:2]))) 
})
...
filename <- paste0('C:/Users/Q/Dropbox/nfi/Cluster/stan/scaleddata22Nov',i,'.R')
