// Model 5: Stabilizing niche difference model
...
model {
	// Likelihood
	for (j in 1:Nspp) {
		// Pairwise interaction coefficients that depend on absolute trait differences of each species, measured as traits.
		// This allows us to model stabilizing niche differences, as compared to model 4, which models relative fitness differences.
		lambda[j] ~ normal(T * sqrt((trait[targetsp][1] - trait[j][1])^2 + (trait[targetsp][2] - trait[j][2])^2), sigma_lambda);
	}	
	for (i in 1:N) {
